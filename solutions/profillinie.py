...
def profile_lv95(punkt1, punkt2, n):
    ...
    Args:
        ...
        n: Integer. Numer of points for the returned DataFrame.
...
def generate_folium_map(df_to_plot):
    ...
    map = folium.Map(location=[46.170018, 8.77492038], zoom_start=8, crs="EPSG3857")

    for _, row in df_to_plot.iterrows():
        folium.CircleMarker(location=[row["Northing"], row["Easting"]], radius=5).add_to(map)
...
def plot_Hoehenprofil(df_to_plot):
    ...
    """
    ax = df_to_plot.plot(x='km from start', y='Z', use_index=True, figsize=(15, 15))
    ax.set_aspect(8)
    fig = ax.get_figure()
